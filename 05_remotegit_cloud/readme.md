...
You can deploy [this script](./azure_arm_free_app_and_database.json) directly using the Azure Portal: 
...
The GitHub workflow (included in the [Starter Template](https://github.com/AvansICT/ICT1.3-2D-Graphics-Secure-Communication-SecureBackend-StarterTemplate)) takes the outputs from the Azure ARM template and the API project folder and uses it to create a simple CI/CD workflow through GitHub actions.
...
* WEBAPI_PROJECT_FOLDER 👉 The folder name of the project that needs to be published (typically MySecureBackend.WebApi).
...
Execute [this script](./create_database_user.sql) on the Azure SQL Database to create a contained SQL database user with only read/write access.
